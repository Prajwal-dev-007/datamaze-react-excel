...
export default function SearchResults() {
    ...
    const handleSelection = (columnKey, value) => {
        const newSelectedValues = {
            ...selectedValues,
            [columnKey]: selectedValues[columnKey] === value ? null : value,
        };

        setSelectedValues(newSelectedValues);

        const matchedRows = data.filter(row =>
            Object.entries(newSelectedValues).every(([key, selectedValue]) =>
        ...
        const queryString = Object.entries(newSelectedValues)
    ...
    const getFilteredValues = (columnKey) => {
        ...
        return [...new Set(highlightedRows.map(row => row[columnKey]))];
    };
                    ...
                    {Object.keys(uniqueValues).map((key, index) => (
                        <div key={index} className={`box${index + 1}`}>
                            ...
                            <div className="value-container">
                                {getFilteredValues(key).map((value, idx) => (
                                    <div
                                        key={idx}
                                        className={`value-box ${
                                            selectedValues[key] === value ? 'highlight' : 'filtered'
                                        }`}
